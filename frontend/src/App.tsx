        ...
        <div className="flex items-center gap-1">
          ...
          {user.isAdmin && (
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${activeTab === 'departments'
                  ? 'bg-blue-50 text-blue-600'
                  : 'text-slate-500 hover:text-slate-800 hover:bg-slate-100'
                }`}
            >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="text-left px-6 py-3.5 font-semibold text-slate-600 text-xs uppercase tracking-wide hidden md:table-cell">Colaboradores</th>
                <th className="px-6 py-3.5"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
...
function App() {
  ...
  const [activeTab, setActiveTab] = useState<'inventory' | 'employees' | 'departments'>('inventory');
